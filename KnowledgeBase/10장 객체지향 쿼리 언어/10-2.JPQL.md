...
for(Object[] row : resultList){
    Member member = (Member) orw[0];    //엔티티
    Product product = (Product) orw[1]; //엔티티
    int orderAmount = (Integer) orw[2]; //스칼라
}
```
...
결과가 아주 많다면 통계 결과만 저장하는 테이블을 별도로 만들고 사용자가 적은 새벽에 통계 쿼릴르 실행해서 그 결과를 보관하는 것이 좋다.
...
- **페치 조인 대상에는 별칭을 줄 수 없다.**   
    ...
    특히 2차 캐시와 함께 사용할 때 조심해야 하는데, 연관된 데이터 수가 다라진 상태에서 2차 캐시에 저장되면 다른 곳에서 조회할 때도 연관된 데이터 수가 달라지는 문제가 발생할 수 있다.
    </details>
...
```
하이버네이트 구현체를 사용하려면 방언 클래스를 상속해서 구현하고 사용할 데이터에비스 함수를 미리 등록해야 한다.   
```java
//방언 클래스 상속
